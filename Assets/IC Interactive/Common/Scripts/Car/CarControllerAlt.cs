...
public class CarControllerAlt : MonoBehaviour {
	...
	void Update () 
	{
		if (canControl) {
			...
			// Handbrake
			handbrake = Mathf.Clamp01 ( handbrake + (Input.GetKey (KeyCode.Mouse2)? Time.deltaTime: -Time.deltaTime) );
			...
			throttle = 0f;
			brake = 0f;
			drivetrain.throttle = 0f;
			drivetrain.throttleInput = 0f;
		}
	}
